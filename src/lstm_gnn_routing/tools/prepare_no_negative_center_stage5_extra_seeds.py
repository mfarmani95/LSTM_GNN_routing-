...
TEMPLATE_CONFIG = PROJECT_ROOT / "runs/No_Negative/center_stage5/seed358881/config.yml"
VALID_SCALER = (
    PROJECT_ROOT
    / "scalers/noah_post_transfer_runoff_scaler_batch_ablation_weighted_mse_seed777_bs1_tgb64.yml"
)
SEEDS = [128604, 219377, 346815, 472930, 538661, 684205, 756119, 827446, 914732, 989014]
...
def _sbatch_text(config_path: Path, account: str, seed: int) -> str:
    log_dir = GRAPH_ROOT / "logs/no_negative_center_stage5_extra_seeds"
    return f"""#!/bin/bash
#SBATCH --job-name=nn_cstage5_{seed}
...
#SBATCH --output={log_dir}/nn_cstage5_{seed}_%j.out
#SBATCH --error={log_dir}/nn_cstage5_{seed}_%j.err
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Prepare 10 extra No_Negative/center_stage5 seed runs.")
    parser.add_argument("--config-root", type=Path, default=PROJECT_ROOT / "configs/no_negative_center_stage5_extra_seeds")
    parser.add_argument("--run-root", type=Path, default=PROJECT_ROOT / "runs/No_Negative/center_stage5")
    parser.add_argument("--sbatch-root", type=Path, default=GRAPH_ROOT / "sbatch_no_negative_center_stage5_extra_seeds")
    ...
    for seed, account in zip(SEEDS, ACCOUNTS):
        ...
        cfg["experiment_name"] = f"no_negative_center_stage5_split619873_seed{seed}"
        ...
        sbatch_path = args.sbatch_root / f"Ocelote_train_no_negative_center_stage5_seed{seed}.sh"
    ...
    submit_script = GRAPH_ROOT / "Ocelote_submit_no_negative_center_stage5_extra_seeds.sh"
    ...
    audit_path = GRAPH_ROOT / "no_negative_center_stage5_extra_seeds_audit.csv"
