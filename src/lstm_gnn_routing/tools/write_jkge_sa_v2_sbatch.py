"""Write non-array sbatch scripts for JKGE_SA_V2 reruns."""
...
SCRIPT_DIR = PROJECT_ROOT / "sbatch_jkge_sa_v2"
...
def _script_text(account: str, job_index: int, rows: list[dict[str, str]]) -> str:
    ...
    for row in rows:
        ...
        commands.append(
            "\n".join(
                [
                    f"echo \"Training JKGE_SA_V2 {label}\"",
...
#SBATCH --job-name=jsa2_{account[:3]}_{job_index:02d}
...
#SBATCH --output={LOG_DIR}/jkge_sa_v2_{account}_{job_index:02d}_%j.out
#SBATCH --error={LOG_DIR}/jkge_sa_v2_{account}_{job_index:02d}_%j.err
...
cd {REPO_ROOT}
echo "Starting JKGE_SA_V2 grouped job {job_index:02d} for account {account}"
...
def main() -> None:
    ...
    if len(rows) != 80:
        raise ValueError(f"Expected 80 JKGE_SA_V2 configs, found {len(rows)}")
    ...
    for account in ACCOUNTS:
        for job_index in range(JOBS_PER_ACCOUNT):
            ...
            if not job_rows:
                continue
            script_path = SCRIPT_DIR / f"Ocelote_train_jkge_sa_v2_{account}_{job_index:02d}.sh"
            script_path.write_text(_script_text(account, job_index, job_rows), encoding="utf-8")
            submit_lines.append(f"sbatch {script_path}")

    submit_path = PROJECT_ROOT / "Ocelote_submit_train_jkge_sa_v2.sh"
