"""Generate JKGE_MA_V1 rerun configs from the JKGE_SA_V2 experiment set."""
...
def _ma_config_from_sa_path(path: Path) -> tuple[Path, Path, str]:
    text = str(path)
    config_file = Path(text.replace("/configs/jkge_sa_v2/", "/configs/jkge_ma_v1/").replace("jkge_sa_v2_", "jkge_ma_v1_"))
    run_dir_text = text.replace("/configs/jkge_sa_v2/", "/runs/jkge_ma_v1/")
    run_dir_text = run_dir_text.rsplit("/", 1)[0]
    # Remove the repeated candidate-specific config filename directories by
    # deriving run_dir from the existing config content later when possible.
    return config_file, Path(run_dir_text), config_file.stem


def _target_run_dir(old_run_dir: str) -> Path:
    return Path(old_run_dir.replace("/runs/jkge_sa_v2/", "/runs/jkge_ma_v1/"))
...
def main() -> None:
    ...
    for row in rows:
        ...
        training["loss"] = "jkge_ma_v1"
        training["jkge_ma_v1_window_length"] = 31
        training["jkge_ma_v1_eps"] = 1.0e-8
        ...
        run_dir = _target_run_dir(str(cfg["run_dir"]))
        cfg["run_dir"] = str(run_dir)
        cfg["experiment_name"] = str(cfg.get("experiment_name", "")).replace("jkge_sa_v2", "jkge_ma_v1")
        _install_private_scaler(cfg, run_dir, reference_scaler)

        config_file, _, _ = _ma_config_from_sa_path(source_config)
    ...
    manifest = REPO_ROOT / "configs/jkge_ma_v1/jkge_ma_v1_manifest.csv"
    manifest.parent.mkdir(parents=True, exist_ok=True)
    manifest.write_text("\n".join(manifest_rows) + "\n", encoding="utf-8")
    print(f"Wrote {count} JKGE_MA_V1 configs")
