...
TEMPLATE_CONFIG = PROJECT_ROOT / "runs/lag/lag30/weighted_mse/seed211329/config.yml"
VALID_SCALER = (
    PROJECT_ROOT
    / "scalers/noah_post_transfer_runoff_scaler_batch_ablation_weighted_mse_seed777_bs1_tgb64.yml"
)
SEEDS = [142783, 235906, 367441, 418625, 584039, 692714, 745280, 831952, 906337, 972418]
ACCOUNTS = ["behrangi", "behrangi", "behrangi", "behrangi", "andrbenn", "andrbenn", "andrbenn", "niug", "niug", "niug"]
...
def _sbatch_text(config_path: Path, account: str, seed: int) -> str:
    log_dir = GRAPH_ROOT / "logs/lag30_weighted_mse_extra_seeds"
    return f"""#!/bin/bash
#SBATCH --job-name=lag30_wmse_{seed}
...
#SBATCH --output={log_dir}/lag30_wmse_{seed}_%j.out
#SBATCH --error={log_dir}/lag30_wmse_{seed}_%j.err
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Prepare 10 extra lag/lag30/weighted_mse seed runs.")
    parser.add_argument("--config-root", type=Path, default=PROJECT_ROOT / "configs/lag30_weighted_mse_extra_seeds")
    parser.add_argument("--run-root", type=Path, default=PROJECT_ROOT / "runs/lag/lag30/weighted_mse")
    parser.add_argument("--sbatch-root", type=Path, default=GRAPH_ROOT / "sbatch_lag30_weighted_mse_extra_seeds")
    ...
    for seed, account in zip(SEEDS, ACCOUNTS):
        ...
        cfg["experiment_name"] = f"lag_lag30_weighted_mse_seed{seed}"
        ...
        sbatch_path = args.sbatch_root / f"Ocelote_train_lag30_weighted_mse_seed{seed}.sh"
        ...
        audit_rows.append(
            {
                ...
                "runoff_lags": repr(cfg.get("routing_model", {}).get("runoff_lags")),
                "routing_lag_context_days": str(cfg.get("routing_model", {}).get("routing_lag_context_days")),
            }
        )

    submit_script = GRAPH_ROOT / "Ocelote_submit_lag30_weighted_mse_extra_seeds.sh"
    ...
    audit_path = GRAPH_ROOT / "lag30_weighted_mse_extra_seeds_audit.csv"
