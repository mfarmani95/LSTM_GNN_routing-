"""Write non-array sbatch scripts for JKGE_MA_V1 reruns."""
...
SCRIPT_DIR = PROJECT_ROOT / "sbatch_jkge_ma_v1"
...
def _script_text(account: str, job_index: int, rows: list[dict[str, str]]) -> str:
    ...
    for row in rows:
        ...
        commands.append(
            "\n".join(
                [
                    f"echo \"Training JKGE_MA_V1 {label}\"",
...
#SBATCH --job-name=jma1_{account[:3]}_{job_index:02d}
...
#SBATCH --output={LOG_DIR}/jkge_ma_v1_{account}_{job_index:02d}_%j.out
#SBATCH --error={LOG_DIR}/jkge_ma_v1_{account}_{job_index:02d}_%j.err
...
cd {REPO_ROOT}
echo "Starting JKGE_MA_V1 grouped job {job_index:02d} for account {account}"
...
def main() -> None:
    ...
    if len(rows) != 80:
        raise ValueError(f"Expected 80 JKGE_MA_V1 configs, found {len(rows)}")
    ...
    for account in ACCOUNTS:
        for job_index in range(JOBS_PER_ACCOUNT):
            ...
            if not job_rows:
                continue
            script_path = SCRIPT_DIR / f"Ocelote_train_jkge_ma_v1_{account}_{job_index:02d}.sh"
            script_path.write_text(_script_text(account, job_index, job_rows), encoding="utf-8")
            submit_lines.append(f"sbatch {script_path}")

    submit_path = PROJECT_ROOT / "Ocelote_submit_train_jkge_ma_v1.sh"
